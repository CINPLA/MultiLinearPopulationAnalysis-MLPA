function [Z_prime, X_firing, X_firing_conv,  weight_evolution, spatial, W] = ...
    sim_tensor_4pop_linear(Rank_1_approx,noise_level)
    
    %------------------------------------------------------------------
    %Changes from sim_tensor_4pop_feed_forward.m:
    %This function uses real Kernels & Approximations
    %------------------------------------------------------------------
    ...
    % Compute firing
    [X, ~] = create_population_tensor(W, tau, f, t, ft);
    X_firing = X;
...
function [population_tensor, stimulus_tensor] = create_population_tensor(W, tau, f, t, ft)
    for i=1:size(W, 1)
        %d = rand(1,4);
        d = ones(1,4);
        mu_0 = d .* f.'; 
        ...
        y= create_single_trial(squeeze(W(i,:,:)), tau, mu, t, ft);
...
function single_trial = create_single_trial(W, tau, mu, t, ft)
    tau = power(tau, -1);
    tspan = t;
    y0 = [0 0 0 0];
    options=odeset('RelTol',1e-4,'AbsTol',1e-6);
    [t,y] = ode45(@(t,y) myODE(t, y, W, tau, mu, ft),tspan, y0, options);
    single_trial = y;
end

function dydt = myODE(t, y, W, tau, mu, ft)
    f = interp1(ft, mu, t);
    dydt = zeros(4,1);
    dydt = tau .* (-1*y + ReLu(W*y + f.'));
end

function r_plus = ReLu(x)
    r_plus = x .* (x>0);
end
